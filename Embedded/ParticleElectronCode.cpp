...
const char *eventName = "isPothole";
...
const unsigned long TIME_AFTER_PUBLISH_MS = 1000; // After publish, wait 1000ms for data to go out
const unsigned long TIME_AFTER_BOOT_MS = 1000; // At boot, wait 10 seconds before going to sleep again
	...
	switch(state) {
    	...
    	case RESET_WAIT_STATE:
    		...
    		if (status != 0) {
    			...
    			// Map the AWAKE bit to INT1 so activity can wake up the Electron
    	...
    	case PUBLISH_STATE:
    		if (Particle.connected()) {
    			...
    			snprintf(data, sizeof(data), "GPS Location");
